...
function NavTrigger({
  ...
  const cls = `relative inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
    open ? "text-foreground font-semibold" : "text-muted-foreground hover:text-foreground"
  }`
...
function NavPanel({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={`absolute top-full left-1/2 z-50 mt-2 -translate-x-1/2 transition-all duration-200 ease-out ${
            ...
            {item.featured && (
              <div className="col-span-2 flex items-center justify-between gap-4 border-t border-border/60 bg-muted/40 px-5 py-3.5 transition-colors hover:bg-muted/60">
                ...
                >
                  ...
                  <span aria-hidden="true" className="transition-transform duration-150 group-hover/feat:translate-x-0.5">
                    →
                  </span>
                </a>
              </div>
                  ...
                  <ul className="space-y-0.5">
                    {g.links.map((l) => (
                      <li key={l.href}>
                        <a
                          href={l.href}
                          className="block rounded-lg px-2.5 py-2 text-sm font-normal text-muted-foreground transition-colors hover:bg-muted hover:font-medium hover:text-foreground"
                        >
                          {l.label}
                        </a>
                      </li>
  ...
  return (
    <div className="border-b border-border/70 last:border-0">
      <button
        ...
        onClick={onToggle}
        className="flex w-full items-center justify-between rounded-lg px-3 py-3 text-left text-[15px] font-medium text-foreground transition-colors hover:bg-muted"
      >
        {item.label}
                  ...
                  {g.links.map((l) => (
                    <li key={l.href}>
                      <a
                        href={l.href}
                        onClick={onNavigate}
                        className="block rounded-lg px-2.5 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                      >
                        {l.label}
                      </a>
                    </li>
...
export default function Navbar() {
  ...
  return (
    <header data-nav-root className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-200 ${scrolled || open ? "border-border bg-background/85 shadow-sm backdrop-blur-xl backdrop-saturate-150" : "border-transparent bg-transparent"}`}>
      <nav className="mx-auto flex h-16 max-w-7xl items-center gap-6 px-6 lg:h-17" aria-label="Primary">
        ...
        <div className="relative ml-4 hidden items-center gap-1 lg:flex" onMouseLeave={clearIndicator}>
          <span
            ...
            style={{ left: indicator ? indicator.x : 0, width: indicator ? indicator.w : 0, opacity: indicator ? 1 : 0 }}
          />
          {NAV_ITEMS.map((item) => (
            <div
              key={item.id}
      ...
      {mobileOpen && (
        <div
          id="mobile-nav"
          className="fixed inset-x-0 top-16 bottom-0 z-40 flex h-[calc(100vh-4rem)] flex-col overflow-y-auto border-t border-border bg-background px-4 pb-24 pt-3 lg:hidden"
        >
